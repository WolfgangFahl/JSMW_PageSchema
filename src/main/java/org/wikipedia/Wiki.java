...
public class Wiki implements Serializable {
	 ...
	 * for the title. Example: en.wikipedia = true, en.wiktionary = false. Default
	...
	public void setMaxLag(int lag) {
		maxlag = lag;
		log(Level.CONFIG, "setMaxLag", "Setting maximum allowable database lag to "
				+ lag);
		initVars();
	}
	...
	public void setStatusCheckInterval(int interval) {
		...
		log(Level.CONFIG, "setStatusCheckInterval", "Status check interval set to "
				+ interval);
	}
	...
	/**
	 * Logs in to the wiki. This method is thread-safe. If the specified username
	 * or password is incorrect, the thread blocks for 20 seconds then throws an
	 * exception.
	 *
	 * @param username
	 *          a username
	...
	public synchronized void login(String username, char[] password)
		...
		// post login request
		// FIXME - this will call the api too early ..
	 ...
	 * to log in again instead of just reading in a serialized wiki. Equivalent to
	 * [[Special:Userlogout]]. This method is thread safe (so that we don't log
	 * out during an edit). WARNING: kills all concurrent sessions - if you are
	 * logged in with a browser this will log you out there as well.
	 *
	 * @throws IOException
	 *           if a network error occurs
	 * @since 0.14
	...
	/**
	 * Determines whether the current user has new messages. (A human would notice
	 * a yellow bar at the top of the page).
	 * 
	...
	protected String parseAndCleanup(String in) throws IOException {
		...
		output = output.replace("<p>", "").replace("</p>", ""); // remove paragraph
																														// tags
	 ...
	 * 		&quot;All orphaned articles&quot;, Wiki.MAIN_NAMESPACE),
	 * 		wikipedia.getCategoryMembers(&quot;All pages needing to be wikified&quot;,
	 * 				Wiki.MAIN_NAMESPACE));
	 * 
	 ...
	 * MediaWiki namespace).
	 * <li><b>token</b>: (String) an edit token for the page, must be logged in to
	 * be non-trivial
	...
	public HashMap[] getPageInfo(String[] pages) throws IOException {
		...
		for (String temp : titles) {
			...
			// <protection />
			// </page>
			for (int j = line.indexOf("<page "); j > 0; j = line.indexOf("<page ",
					++j)) {
				...
				if (exists) {
					tempmap.put("lastpurged",
							timestampToCalendar(parseAttribute(item, "touched", 0), true));
					tempmap.put("lastrevid",
							Long.parseLong(parseAttribute(item, "lastrevid", 0)));
					tempmap.put("size",
				...
				if (item.contains("watchers=\""))
					tempmap.put("watchers",
							Integer.parseInt(parseAttribute(item, "watchers", 0)));
	 ...
	 * thread safe and blocks for a minimum time as specified by the throttle. The
	 * edit will be marked bot if <tt>isMarkBot() == true</tt> and minor if
	...
	public String[] getImagesOnPage(String title) throws IOException {
		...
		log(Level.INFO, "getImagesOnPage", "Successfully retrieved images used on "
				+ title + " (" + temp + " images)");
	 ...
	 * categories. Capped at <tt>max</tt> number of categories, there's no reason
	 * why there should be more than that.
	 *
	 * @param title
	 *          a page
	 ...
	 * if sortkey is true. The sortkey would then be appended to the element of
	 * the returned string array (separated by "|"). Capped at <tt>max</tt> number
	 * of categories, there's no reason why there should be more than that.
	 * 
	 * @param title
	 *          a page
	 * @param sortkey
	...
	public String[] getCategories(String title, boolean sortkey,
		...
		for (a = line.indexOf("<cl "); a > 0; a = b) {
			...
			if (ignoreHidden
					&& line.substring(a, (b > 0 ? b : line.length())).contains("hidden"))
				continue;
		...
		log(Level.INFO, "getCategories", "Successfully retrieved categories of "
	...
	public String[] getTemplates(String title, int... ns) throws IOException {
		...
		log(Level.INFO, "getTemplates", "Successfully retrieved templates used on "
	...
	public String[] getLinksOnPage(String title) throws IOException {
			...
			if (plcontinue == null)
				...
				line = fetch(
						url.toString() + "&plcontinue="
								+ URLEncoder.encode(plcontinue, "UTF-8"), "getLinksOnPage");
		...
		int size = links.size();
		log(Level.INFO, "getLinksOnPage", "Successfully retrieved links used on "
				+ title + " (" + size + " links)");
	...
	public String[] getExternalLinksOnPage(String title) throws IOException {
		...
		log(Level.INFO, "getExternalLinksOnPage",
				"Successfully retrieved external links used on " + title + " (" + size
						+ " links)");
		return links.toArray(new String[size]);
	}
	...
	public LinkedHashMap<String, String> getSectionMap(String page)
		...
		log(Level.INFO, "getSectionMap", "Successfully retrieved section map for "
				+ page);
		return map;
	}
	...
	protected Revision[] deletedRevs(String u, String title, Calendar start,
		...
		do {
			...
			if (drcontinue != null)
				response = fetch(
						url.toString() + "&drcontinue="
								+ URLEncoder.encode(drcontinue, "UTF-8"), "deletedRevs"); // huh?
			...
			if (x < 0) // no deleted history/contributions
				break;
			for (x = response.indexOf("<page ", x); x > 0; x = response.indexOf(
					"<page ", ++x)) {
	...
	public String export(String title) throws IOException {
		return fetch(
				query + "export&exportnowrap&titles="
						+ URLEncoder.encode(normalize(title), "UTF-8"), "export");
	}
	...
	public Revision[] getRevisions(long[] oldids) throws IOException {
		...
		for (String chunk : chunks) {
			...
			for (int i = line.indexOf("<page "); i > 0; i = line.indexOf("<page ",
					++i)) {
	...
	public synchronized void rollback(Revision revision, boolean bot,
		...
		if (retry)
			log(Level.INFO, "rollback", "Successfully reverted edits by " + user
					+ " on " + revision.getPage());
		retry = true;
	}
	...
	public synchronized void revisionDelete(Boolean hidecontent,
			Boolean hideuser, Boolean hidereason, String reason, Boolean suppress,
			Revision[] revisions) throws IOException, LoginException {
		long start = System.currentTimeMillis();
		...
		// send/read response
		String response = post(apiUrl + "action=revisiondelete", out.toString(),
				"revisionDelete");
		try {
			// success
			...
			if (retry) {
				retry = false;
				log(Level.WARNING, "revisionDelete", "Exception: " + e.getMessage()
						+ " Retrying...");
				revisionDelete(hidecontent, hideuser, hidereason, reason, suppress,
						revisions);
			} else {
	...
	protected Revision parseRevision(String xml, String title) {
		...
		Revision revision = new Revision(oldid, timestamp, title, summary, user2,
				minor, bot, rvnew, size);
		// set rcid
	 ...
	 * directly into a <tt>BufferedImage</tt> object, so you won't be able to get
	 * all metadata that way. The keys are:
	 *
	...
	public String[] getDuplicates(String file) throws IOException {
		...
		log(Level.INFO, "getDuplicates",
				"Successfully retrieved duplicates of File:" + file + " (" + size
						+ " files)");
	...
	public byte[] getOldImage(LogEntry entry) throws IOException {
		...
		for (int a = line.indexOf("<ii "); a > 0; a = line.indexOf("<ii ", ++a)) {
			...
			if (timestamp.equals(calendarToTimestamp(entry.getTimestamp()))) {
				...
				if (archive == null)
					archive = title;
				log(Level.INFO, "getOldImage", "Successfully retrieved old image \""
						+ archive + "\"");
				return out.toByteArray();
			}
		}
	...
	public LogEntry[] getUploads(User user, Calendar start, Calendar end)
		...
		log(Level.INFO, "getUploads",
				"Successfully retrieved uploads of " + user.getUsername() + " (" + size
						+ " uploads)");
	 ...
	 * are (case-insensitive) "png", "jpg", "gif" and "svg". You need to be logged
	 * on to do this. Automatically breaks uploads into 2^<tt>LOG2_CHUNK_SIZE</tt>
	 * byte size chunks. This method is thread safe and subject to the throttle.
	 *
	 * @param file
	 *          the image file
	 * @param filename
	 ...
	 * @since 0.21
	 */
	public synchronized void upload(File file, String filename, String contents,
			String reason) throws IOException, LoginException {
		// TODO: upload via URL
		...
		for (int i = 0; i < chunks; i++) {
			...
			if (chunks == 1) {
				...
				byte[] by = new byte[(int) buffersize]; // 32 bit problem. Why must
																								// array indices be ints?
				fi.read(by);
			...
			try {
				...
				if (chunks > 1) {
					if (response.contains("filekey=\"")) {
						...
						throw new IOException("No filekey present! Server response was "
								+ response);
				}
	...
	public String[] allUsers(String start, int number, String prefix)
		...
		ArrayList<String> members = new ArrayList<String>(6667); // enough for most
																															// requests
		do {
	...
	/**
	 * Gets the contributions by a range of IP v4 addresses. Supported ranges are
	 * /8, /16 and /24. Do be careful with this, as calls such as
	...
	public Revision[] rangeContribs(String range) throws IOException {
		...
		case 24:
			return contribs("", numbers[0] + "." + numbers[1] + "." + numbers[2]
					+ ".", null, null);
	...
	public Revision[] contribs(String user, String prefix, Calendar end,
		...
		do {
			...
			for (int a = line.indexOf("<item "); a > 0; a = line.indexOf("<item ",
					++a)) {
				int b = line.indexOf(" />", a);
	 ...
	 * @since 0.24
	 */
	public synchronized void emailUser(User user, String message, String subject,
	...
	public void watch(String... titles) throws IOException,
			CredentialNotFoundException {
		/*
		 * Ideally, we would have a setRawWatchlist() equivalent in the API, and as
		 * such not have to send title.length requests. Then we can do away with
	...
	public String[] getRawWatchlist(boolean cache) throws IOException,
		...
		do {
			...
			if (wrcontinue == null)
				...
				line = fetch(
						url + "&wrcontinue=" + URLEncoder.encode(wrcontinue, "UTF-8"),
						"getRawWatchlist");
		...
		log(Level.INFO, "getRawWatchlist", "Successfully retrieved raw watchlist ("
				+ size + " items)");
	...
	public Revision[] watchlist(boolean allrev, int... ns) throws IOException,
		...
		do {
			...
			for (int i = line.indexOf("<item "); i > 0; i = line.indexOf("<item ",
					++i)) {
		...
		int size = wl.size();
		log(Level.INFO, "watchlist", "Successfully retrieved watchlist (" + size
				+ " items)");
	...
	public String[][] search(String search, int... namespaces) throws IOException {
		...
		log(Level.INFO, "search", "Successfully searched for string \"" + search
	...
	public String[] whatLinksHere(String title, boolean redirects, int... ns)
		...
		log(Level.INFO, "whatLinksHere", "Successfully retrieved "
				+ (redirects ? "redirects to " : "links to ") + title + " (" + size
				+ " items)");
		return pages.toArray(new String[size]);
	}
	...
	public String[] whatTranscludesHere(String title, int... ns)
		...
		log(Level.INFO, "whatTranscludesHere",
				"Successfully retrieved transclusions of " + title + " (" + size
						+ " items)");
		return pages.toArray(new String[size]);
	}
	...
	public String[] getCategoryMembers(String name, boolean subcat, int... ns)
		...
		log(Level.INFO, "getCategoryMembers",
				"Successfully retrieved contents of Category:" + name + " (" + size
						+ " items)");
		return members.toArray(new String[size]);
	}
	 ...
	 * Returns two lists, where the first is the list of pages and the second is
	 * the list of urls. The index of a page in the first list corresponds to the
	 * index of the url on that page in the second list. Wildcards (*) are only
	 ...
	 * @since 0.12
	 */
	protected LogEntry[] getIPBlockList(String user, Calendar start, Calendar end)
			throws IOException {
		// quick param check
		...
		do {
			...
			// parse xml
			for (int a = line.indexOf("<block "); a > 0; a = line.indexOf("<block ",
					++a)) {
				// find entry
				int b = line.indexOf("/>", a);
	...
	/**
	 * Gets the most recent set of log entries up to the given amount. Equivalent
	 * to [[Special:Log]].
	 *
	 * @param amount
	 ...
	 * @param action
	 *          what action to get (e.g. delete, undelete, etc.), use "" to not
	 *          specify one
	 * @throws IOException
	 *           if a network error occurs
	 * @throws IllegalArgumentException
	 ...
	 * given user on the given target. Equivalent to [[Special:Log]]. WARNING: the
	 * start date is the most recent of the dates given, and the order of
	...
	public LogEntry[] getLogEntries(Calendar start, Calendar end, int amount,
		...
		ArrayList<LogEntry> entries = new ArrayList<LogEntry>(6667); // should be
																																	// enough
		do {
			...
			// parse xml. We need to repeat the test because the XML may contain more
	...
	protected LogEntry parseLogEntry(String xml) {
		...
		if (xml.contains("commenthidden=\""))
			reason = null;
		else if (type.equals(USER_CREATION_LOG)) // there is no reason for creating
																							// a user
			reason = "";
	 ...
	 * and [[Special:Allmessages]] (if namespace == MEDIAWIKI_NAMESPACE). WARNING:
	 * Limited to 500 values (5000 for bots), unless a prefix or protection level
	 * is specified.
	 *
	 * @param prefix
	 ...
	 */
	public String[] listPages(String prefix,
			HashMap<String, Object> protectionstate, int namespace, int minimum,
		...
		// tidy up
		int size = pages.size();
		log(Level.INFO, "listPages", "Successfully retrieved page list (" + size
				+ " pages)");
		return pages.toArray(new String[size]);
	}
	 ...
	 *          Unwatchedpages, Unusedtemplates, Withoutinterwiki }. This
	...
	public String[] queryPage(String page) throws IOException,
		...
		do {
			...
			for (int x = line.indexOf("<page "); x > 0; x = line.indexOf("<page ",
					++x))
				pages.add(decode(parseAttribute(line, "title", x)));
		} while (offset != null);
		int temp = pages.size();
	 ...
	 * table only stores new pages for about a month. It is not possible to
	 * retrieve changes before then. Equivalent to [[Special:Newpages]].
	 *
	 * @param rcoptions
	...
	protected Revision[] recentChanges(int amount, int rcoptions,
		...
		log(Level.INFO, "recentChanges", "Successfully retrieved recent changes ("
				+ temp + " revisions)");
...
/**
		...
		public HashMap<String, Object> getUserInfo() throws IOException {
			...
			ret.put("created",
					timestampToCalendar(parseAttribute(info, "registration", 0), true));
		...
		/**
		 * Returns true if the user is allowed to perform the specified action. Uses
		 * the rights cache. Read [[Special:Listgrouprights]] before using this!
		 * 
		 * @param right
		 *          a specific action
	...
	public class LogEntry implements Comparable<LogEntry> {
		 ...
		 * @param details
		 *          the details of the action (e.g. the new title of the page after
		 ...
		 * <tr>
		 * <td>PROTECTION_LOG
		 * <td>action == "protect" or "modify" => the protection level (int, -2 if
		 * unrecognized), action == "move_prot" => the old title, else null
		 * <tr>
		 * <td>Others or RevisionDeleted
		 * <td>null
		 * </table>
		 *
		 * Note that the duration of a block may be given as a period of time (e.g.
		 * "31 hours") or a timestamp (e.g. 20071216160302). To tell these apart,
		 * feed it into <tt>Long.parseLong()</tt> and catch any resulting
		 * exceptions.
		 *
	...
	public class Revision implements Comparable<Revision> {
		 ...
		 * 
		 * @param revid
		 *          the id of the revision (this is a long since {{NUMBEROFEDITS}}
		 *          on en.wikipedia.org is now (January 2012) ~25% of
		...
		public Revision(long revid, Calendar timestamp, String title,
				String summary, String user, boolean minor, boolean bot, boolean rvnew,
				int size) {
			this.revid = revid;
		...
		/**
		 * Fetches the contents of this revision. WARNING: fails if the revision is
		 * deleted.
		 * 
		...
		/**
		 * Gets the rendered text of this revision. WARNING: fails if the revision
		 * is deleted.
		 * 
		 ...
		 * WARNING: Returning true does not imply this is the bottommost revision on
		 * the page due to histmerges.<br>
		...
		/**
		 * Returns the edit summary for this revision, or null if we cannot access
		 * it.
		 * 
		...
		/**
		 * Returns the user or anon who created this revision. You should pass this
		 * (if not an IP) to <tt>getUser(String)</tt> to obtain a User object.
			...
			try {
				synchronized (this) {
					...
					log(Level.WARNING, caller, "Current database lag " + lag
							+ " s exceeds " + maxlag + " s, waiting " + time + " s.");
	 ...
	 * @since 0.10
	 */
	protected boolean checkRights(HashMap<String, Object> pageinfo, String action)
			throws IOException {
	...
	protected void log(Level level, String method, String text) {
		...
		logger.logp(level, "Wiki", method, "[{0}] {1}",
				new Object[] { domain, text });
	}
